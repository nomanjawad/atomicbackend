...
const CsrTable = () => {
  const [csrPages, setCsrPages] = useState([])
  ...
  useEffect(() => { fetchCsrPages() }, [])

  const fetchCsrPages = async () => {
    setIsLoading(true)
    try {
      const response = await csrPagesAPI.getAll()
      if (response.success) setCsrPages(response.data || [])
      else setMessage({ type: "error", text: "Failed to fetch CSR pages" })
    } catch (error) {
      setMessage({ type: "error", text: error.message || "Failed to fetch CSR pages" })
    } finally { setIsLoading(false) }
  }

  const handleEdit = (id) => navigate(`/csr-form?id=${id}`)
  const handleDelete = async (id) => {
    if (!window.confirm("Delete this CSR page?")) return
    try {
      const response = await csrPagesAPI.delete(id)
      if (response.success) {
        setMessage({ type: "success", text: "CSR page deleted" })
        setCsrPages(prev => prev.filter(p => p.id !== id))
  ...
  if (isLoading) return (
    <div className="container-fluid"><div className="card"><div className="card-body d-flex justify-content-center py-5">
    ...
    <div className="container-fluid">
      <div className="card">
        <div className="card-header d-flex justify-content-between align-items-center">
          <h5 className="card-title mb-0">CSR Pages</h5>
          <button className="btn btn-primary btn-sm" onClick={() => navigate("/csr-form")}>
        ...
        <div className="card-body">
          ...
          {csrPages.length === 0 ? (
            <div className="text-center py-5"><Icon icon="heroicons:document-text" className="text-secondary" style={{ fontSize: "48px" }} /><p className="text-secondary mt-3">No CSR pages found.</p></div>
          ) : (
