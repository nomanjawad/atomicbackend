...
const IndustriesPageTable = () => {
  ...
  const fetchPages = async () => {
    setIsLoading(true)
    try {
      const response = await industriesPagesAPI.getAll()
      if (response.success) setPages(response.data || [])
      else setMessage({ type: "error", text: "Failed to fetch industries pages" })
    } catch (error) { setMessage({ type: "error", text: error.message }) }
    finally { setIsLoading(false) }
  }

  const handleEdit = (id) => navigate(`/industries-page-form?id=${id}`)
  const handleDelete = async (id) => {
    if (!window.confirm("Delete this industries page?")) return
    try {
      const response = await industriesPagesAPI.delete(id)
    ...
    <div className="container-fluid">
      <div className="card">
        <div className="card-header d-flex justify-content-between align-items-center">
          <h5 className="card-title mb-0">Industries Pages</h5>
          <button className="btn btn-primary btn-sm" onClick={() => navigate("/industries-page-form")}><Icon icon="heroicons:plus" className="me-1" />Add New</button>
        </div>
        <div className="card-body">
          {message.text && <div className={`alert alert-${message.type === "success" ? "success" : "danger"} mb-3`}>{message.text}</div>}
          {pages.length === 0 ? (
            <div className="text-center py-5"><Icon icon="heroicons:building-office" className="text-secondary" style={{ fontSize: "48px" }} /><p className="text-secondary mt-3">No industries pages found.</p></div>
          ) : (
...
export default IndustriesPageTable
