...
const GalleryPageTable = () => {
  ...
  const fetchPages = async () => {
    setIsLoading(true)
    try {
      const response = await galleryPagesAPI.getAll()
      if (response.success) setPages(response.data || [])
      else setMessage({ type: "error", text: "Failed to fetch gallery pages" })
    } catch (error) { setMessage({ type: "error", text: error.message }) }
    finally { setIsLoading(false) }
  }

  const handleEdit = (id) => navigate(`/gallery-page-form?id=${id}`)
  const handleDelete = async (id) => {
    if (!window.confirm("Delete this gallery page?")) return
    try {
      const response = await galleryPagesAPI.delete(id)
    ...
    <div className="container-fluid">
      <div className="card">
        <div className="card-header d-flex justify-content-between align-items-center">
          <h5 className="card-title mb-0">Gallery Pages</h5>
          <button className="btn btn-primary btn-sm" onClick={() => navigate("/gallery-page-form")}><Icon icon="heroicons:plus" className="me-1" />Add New</button>
        </div>
        <div className="card-body">
          {message.text && <div className={`alert alert-${message.type === "success" ? "success" : "danger"} mb-3`}>{message.text}</div>}
          {pages.length === 0 ? (
            <div className="text-center py-5"><Icon icon="heroicons:photo" className="text-secondary" style={{ fontSize: "48px" }} /><p className="text-secondary mt-3">No gallery pages found.</p></div>
          ) : (
...
export default GalleryPageTable
